	...
	$(function() {
		$('#continue').unbind('click').click(function(data) {
			$.post('data_input.php?action=field_remove', {
				...
				data_input_id: <?php print cacti_js_encode(get_request_var('data_input_id'));?>,
				id: <?php print cacti_js_encode(get_request_var('id'));?>
			}, function(data) {
				loadPageNoHeader('data_input.php?action=edit&header=false&id=<?php print cacti_js_encode(get_request_var('data_input_id'));?>');
			});
		});
	});
	</script>
