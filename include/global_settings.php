...
$settings['general'] = [
	...
	'content_security_policy_script' => [
		...
		'description'   => __('Controls how the Content-Security-Policy restricts inline JavaScript.  HTMLX keeps the \'unsafe-inline\' policy that all current themes and plugins rely on and matches the behaviour of prior releases.  Nonce enforces a per-request nonce on Cacti inline scripts and allows scripts they load (plugins that emit their own inline scripts must adopt the nonce or they will be blocked).  None also permits unsafe eval() calls and is the least restrictive option.'),
		'default'       => '',
		'array'         => [
			''            => __('HTMLX (Default)'),
			'nonce'       => __('Nonce'),
