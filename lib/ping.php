	...
	function ping_icmp() : bool {
		...
		if ($this->host['hostname']) {
			...
			if ($fping != '' && file_exists($fping) && is_executable($fping)) {
				...
				$result = shell_exec($fping . ' -q -t ' . $this->timeout . ' -c 1 -r ' . $this->retries . ' ' . escapeshellarg($host_ip) . ' 2>&1');
			} else {
				$using_fping = false;
				...
				if (substr_count(strtolower(PHP_OS), 'sun')) {
					$result = shell_exec('ping ' . escapeshellarg($host_ip));
				} elseif (substr_count(strtolower(PHP_OS), 'hpux')) {
					$result = shell_exec('ping -m ' . ceil($this->timeout / 1000) . ' -n ' . $this->retries . ' ' . escapeshellarg($host_ip));
				} elseif (substr_count(strtolower(PHP_OS), 'mac')) {
					$result = shell_exec('ping -t ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' ' . escapeshellarg($host_ip));
				} elseif (substr_count(strtolower(PHP_OS), 'freebsd')) {
					if (str_contains($host_ip, ':')) {
						$result = shell_exec('ping6 -t ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' ' . escapeshellarg($host_ip));
					} else {
						$result = shell_exec('ping -t ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' ' . escapeshellarg($host_ip));
					}
					...
					$result = shell_exec('ping -w ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' ' . escapeshellarg($host_ip));
				} elseif (substr_count(strtolower(PHP_OS), 'aix')) {
					$result = shell_exec('ping -i ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' ' . escapeshellarg($host_ip));
				} elseif (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
					$result = shell_exec('chcp 437 && ping -w ' . $this->timeout . ' -n ' . $this->retries . ' ' . escapeshellarg($host_ip));
				} else {
					/**
					 ...
					 */
					if (str_contains($host_ip, ':')) {
						$result = shell_exec('ping -6 -W ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' -p ' . $pattern . ' ' . escapeshellarg($host_ip));
					} else {
						$result = shell_exec('ping -W ' . ceil($this->timeout / 1000) . ' -c ' . $this->retries . ' -p ' . $pattern . ' ' . escapeshellarg($host_ip) . ' 2>&1');
					}
				}
			}
