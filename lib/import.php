...
function xml_to_data_input_method(string $hash, array &$xml_array, array &$hash_cache) : mixed {
	...
	foreach ($fields_data_input_edit as $field_name => $field_array) {
		...
		if (isset($xml_array[$field_name])) {
			...
			// apply the same shell-metacharacter guard the GUI save path uses so
			// a hostile XML/package cannot persist a command-injection payload
			// that the UI would have rejected at data_input.php
